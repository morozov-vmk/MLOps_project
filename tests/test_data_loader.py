...
from data_loader import CashbackDataset, DataProcessor
...
class TestDataProcessor:
    ...
    def test_clean_data_removes_inf_and_nan(self, sample_config, sample_dataframe):
        ...
        assert (
            np.isinf(sample_dataframe.select_dtypes(include=[np.number])).sum().sum()
            > 0
        )
    ...
    def test_validate_data_detects_problems(self, sample_config):
        ...
        assert (
            not np.isinf(validated_data.select_dtypes(include=[np.number])).any().any()
        )
    ...
    def test_data_loader_creation(self, sample_config):
        ...
        for batch in train_loader:
            ...
            assert (
                features.shape[0] == sample_config["training"]["batch_size"]
                or features.shape[0] == 100 % sample_config["training"]["batch_size"]
            )
