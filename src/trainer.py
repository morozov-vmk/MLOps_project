...
class ModelTrainer:
    ...
    def log_metrics(self, epoch, train_loss, val_metrics):
        ...
        logger.info("┌" + "─" * 60 + "┐")
        logger.info(f"│ Epoch {epoch+1:3d}/{self.config['training']['num_epochs']:3d} " + " " * 35 + "│")
        logger.info("├" + "─" * 60 + "┤")
    ...
    def train(self, train_loader, val_loader):
        ...
        logger.info(f"Model will be evaluated on {len(val_loader.dataset)} validation samples")
        logger.info(f"Target metric: AUC (early stopping patience: {self.config['training']['early_stopping_patience']})")
