...
class DataProcessor:
    ...
    def _clean_data(self, df):
        ...
        inf_count_before = np.isinf(df_clean.select_dtypes(include=[np.number])).sum().sum()
        nan_count_before = df_clean.isna().sum().sum()
        
        logger.info(f"Found {inf_count_before} infinite values and {nan_count_before} NaN values before cleaning")
        ...
        inf_count_after = np.isinf(df_clean.select_dtypes(include=[np.number])).sum().sum()
        nan_count_after = df_clean.isna().sum().sum()
        
        logger.info(f"After cleaning: {inf_count_after} infinite values and {nan_count_after} NaN values")
    ...
    def _validate_data(self, df, dataset_name):
        ...
        for col in numerical_cols[:5]:
            ...
            logger.debug(f"{col}: min={col_min:.4f}, max={col_max:.4f}, mean={col_mean:.4f}")
    ...
    def load_and_preprocess_data(self):
        ...
        train_df = pd.read_parquet(self.config["data"]["train_path"]).sample(n=1000000, random_state=self.config["data"]["random_seed"])
        test_df = pd.read_parquet(self.config["data"]["test_path"]).sample(n=1000000, random_state=self.config["data"]["random_seed"])
        ...
        exclude_cols = self.config["data"]["exclude_columns"] + [self.config["data"]["target_column"]]
        ...
        try:
            ...
            iqr = np.percentile(X_train_array, 75, axis=0) - np.percentile(X_train_array, 25, axis=0)
    ...
    def _check_final_data_quality(self, data, name):
        ...
        if isinstance(data, np.ndarray):
            ...
            logger.info(f"{name} - Inf: {has_inf}, NaN: {has_nan}, "
